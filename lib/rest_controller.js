...
(function() {
  var JSONUtils, JoinTableControllerSingleton, ORMUtils, RESTController, bbCallback, _,
  ...
  module.exports = RESTController = (function() {
    ...
    RESTController.prototype.sendError = function(res, err) {
      this.logger.error("Error 500 from " + res.req.method + " " + res.req.url + ": " + err);
    ...
    RESTController.prototype.create = function(req, res) {
      ...
      try {
        ...
        return model.save({}, bbCallback(function(err) {
          if (err) {
            return _this.sendError(res, err);
          }
    ...
    RESTController.prototype.update = function(req, res) {
      ...
      try {
        ...
        return this.model_type.find(req.params.id, function(err, model) {
          ...
          return model.save(model.parse(json), bbCallback(function(err) {
            if (err) {
              return _this.sendError(res, err);
            }
    ...
    RESTController.prototype.preRender = function(req, json, callback) {
      return callback();
    };

    RESTController.prototype.render = function(req, json, callback) {
      var _this = this;
      return this.preRender(req, json, function(err) {
        var models, options, template, template_name;
        if (err) {
          return callback(err);
        }
        template_name = req.query.$render || req.query.$template || _this.default_template;
        if (!template_name) {
          return callback(null, json);
        }
        if (!(template = _this.templates[template_name])) {
          return callback(new Error("Unrecognized template: " + template_name));
        }
        options = (_this.renderOptions ? _this.renderOptions(req, template_name) : {});
        ...
        }) : new _this.model_type(_this.model_type.prototype.parse(json));
